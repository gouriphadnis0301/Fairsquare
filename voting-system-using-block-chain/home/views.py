...
def authentication(request):
    ...
    # if no aadhar provided
    if not aadhar_no:
        return JsonResponse({'success': False, 'error': 'Please provide an Aadhar number.'})

    # Try to fetch existing voter, otherwise create a dummy voter
    try:
        voter = Voters.objects.get(uuid=aadhar_no)
    except Voters.DoesNotExist:
        return JsonResponse({'success': False, 'error': 'Aadhar number not found in database.'})

    # store uuid in session for later steps
    request.session['uuid'] = aadhar_no

    # Render candidate details HTML (keeps original behaviour)
    render_html = loader.render_to_string('candidate_details.html', {'details': voter})

    # If already voted, return error; otherwise return success + html + voter data
    if voter.vote_done:
        details = {
            'success' : False,
